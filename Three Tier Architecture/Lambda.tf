...
#IAM policies so Lambda has the permissions to execute the function on the EC2 instances
...
resource "aws_lambda_function" "SnapshotScheduler" {  
  ...
  handler       = "SnapshotSchedulerFunction.lambda_handler"  

    runtime = "python3.9"
}
  
    
#logging for the lambda function
